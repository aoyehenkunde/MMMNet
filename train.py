...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, text_features, image_features, labels):
        ...
        mask = torch.eq(labels, labels.T).float().to(text_features.device)
...
def train():
    ...
    # ---  Load Data  ---
    # The feature vectors we processed are saved in numpy files
    # pic_sample: Randomly shuffled image samples
    # text_fixed: Real text samples
    # pic_matched: Images that match real text
    # text_fake: Fake text samples
    # pic_fake: Fake picture samples
    pic_sample = np.load('')
    text_fixed = np.load('')
    pic_matched = np.load('')
    text_fake = np.load('')
    ...
    # m1: The number of positive samples from the same real news
    # m2: The number of negative samples from the different news
    ...
    # n1: The number of real news
    # n2: The number of fake news
    ...
    # loss_function_auxiliary = nn.CosineEmbeddingLoss(reduction='mean')#cosine_Loss for first stage
    loss_function_auxiliary = InfoNCELoss() # InfoLoss
