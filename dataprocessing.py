...
# Resample the original data and sample according to whether the pictures and texts are from the same news
def resample_pairs(text_features, image_features):
    batch_size = text_features.size(0)

    # Constructing negative sample pairs
    negative_text_features = text_features.clone()
    negative_image_features = image_features.clone()

    # Shuffle image or text features to construct negative samples
    random_indices = torch.randperm(batch_size)
    negative_image_features = negative_image_features[random_indices]

    # Merge positive and negative samples
    combined_text_features = torch.cat([text_features, negative_text_features], dim=0)
    combined_image_features = torch.cat([image_features, negative_image_features], dim=0)

    return combined_text_features, combined_image_features
...
feature_positive, feature_negative = resample_pairs(text_hashCodes, out_a)

# Corresponding to the features in train, all the above features are saved in numpy files
#
# print(feature_out)
